...
usage() {
  cat <<EOF
Install Workroom Harness for Codex into an existing project.

Usage:
  curl -fsSL https://raw.githubusercontent.com/asleworks/workroom-harness/main/.workroom/scripts/install-codex.sh | bash
  curl -fsSL https://raw.githubusercontent.com/asleworks/workroom-harness/main/.workroom/scripts/install-codex.sh | bash -s -- /path/to/project

Options:
  --overwrite  Overwrite existing Workroom Harness files.
...
python3 "$SOURCE_DIR/.workroom/scripts/install.py" "$TARGET" --agent codex $OVERWRITE $DRY_RUN
